...
from .layers import SingleLeftLinear, SingleRightLinear, PreGCN, GC_Block, PostGCN, TransMsr
...
class MSRGCN(nn.Module):
    def __init__(self, p_dropout, leaky_c=0.2, final_out_noden=22, input_feature=35, **dic):
        ...
        # transformer部分
        self.first_trans = nn.Sequential(
            TransMsr(),
            nn.BatchNorm1d(66)
        )
    ...
    def forward(self, inputs):
        ...
        trans_first_left = self.first_trans(enhance_first_left) # 添加trnsformer模块
        out_first_left = self.first_left(trans_first_left) + trans_first_left  # 残差连接
