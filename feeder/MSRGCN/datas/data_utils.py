...
def define_actions(action):
    ...
    if(isinstance(action, list)):
        action = [i for i in action if i in actions]
        return action
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5):
                for subact in [1, 2]:  # subactions
                    ...
                    action_sequence = readCSVasFloat(filename)
                    n, d = action_sequence.shape
                    # print("action_sequence.shape", action_sequence.shape) # 一个txt文件就是一个视频信息，n表示视频的帧数，d为33个点的3维坐标，其中只有22个点有用
                    even_list = range(0, n, sample_rate)
                    ...
                    # print("p3d.shape", p3d.shape) # 形状为：帧数*骨骼点数*坐标维度，表示原始32个点在视频中的xyz坐标
                    the_sequence = p3d.view(num_frames, -1).cpu().data.numpy()
                    # print("the_sequence.shape", the_sequence.shape) # 将xyz坐标拉到骨骼点维度上，形状为：帧数*（骨骼点数乘以坐标维度数）

                    fs = np.arange(0, num_frames - seq_len + 1)
                    fs_sel = fs
                    for i in np.arange(seq_len - 1):
                        fs_sel = np.vstack((fs_sel, fs + i + 1))
                    # print("fs_sel.shape", fs_sel.shape) # 
                    fs_sel = fs_sel.transpose()
                    # print("fs_sel.transpose().shape", fs_sel.shape) # 
                    seq_sel = the_sequence[fs_sel, :]
                ...
                fs_sel1 = [np.arange(i, i + seq_len) for i in range(num_frames1 - 100)] # 直接全取
                ...
                # fs_s1 = fs_sel1
                # fs_s2 = fs_sel2
                # seq_sel1 = the_sequence1[fs_s1, :]
                # seq_sel2 = the_sequence2[fs_s2, :] # xiugai
                if len(sampled_seq) == 0:
                    sampled_seq = seq_sel1
                    sampled_seq = np.concatenate((sampled_seq, seq_sel2), axis=0)
                    complete_seq = the_sequence1
